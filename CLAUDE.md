...
**My fork is `laam21/ai_agent_workshop`.**

Anything that **creates** — `gh issue create`, `gh pr create` — passes
`--repo laam21/ai_agent_workshop`. Never write to
...
**`mytools` is written in `python`.** Every subcommand and every test. Don't
...
Two measured traps that have already caught us — both are in `SPEC.md` with the
evidence, and both look like logic bugs when they fail:

- **`sort` has no `end` key.** bedtools sorts stably on `(chrom, start)` only, so
  equal-start intervals keep input order. `sorted(key=lambda r: (r.chrom, r.start,
  r.end))` is wrong and diverges on `data/a.bed`.
- **`closest -d` is `b.start - a.end + 1`,** not the raw gap. Bookended features report
  `1`; only true overlaps report `0`.

`SPEC.md` is the contract. Read it before implementing a subcommand; anything marked
*Measured* there came from running real bedtools, so don't second-guess it — re-measure
if you doubt it.
